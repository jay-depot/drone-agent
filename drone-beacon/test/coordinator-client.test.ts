...
describe('Coordinator Client', () => {
  ...
  beforeEach(async () => {
    configDir = await mkdtemp(path.join(os.tmpdir(), 'drone-beacon-coordinator-client-'));
    // Mock http.request
    mockRequest = vi.fn();
  ...
  describe('createCoordinatorFetch', () => {
    it('should create a fetch wrapper', async () => {
      const { createCoordinatorFetch } = await import('../src/coordinator-client.js');
  ...
  describe('createCoordinatorClient', () => {
    it('should return a client with all required methods', async () => {
      const { createCoordinatorClient } = await import('../src/coordinator-client.js');
      ...
      const client = createCoordinatorClient(
        { host: 'localhost', port: 3456, beaconId: 'test-beacon', beaconName: 'Test Beacon' },
  ...
  describe('registerBeacon', () => {
    it('should register beacon and return status', async () => {
      ...
      const result = await client.registerBeacon(identity, tlsIdentity.fingerprint);
    ...
    it('should throw on non-ok response', async () => {
      ...
      await expect(client.registerBeacon(identity, tlsIdentity.fingerprint)).rejects.toThrow();
    });
  });
  ...
  describe('fetchPersonas', () => {
    it('should fetch personas and mark them as coordinator scope', async () => {
      ...
      setupMockHttpResponse(200, [
        { id: 'p1', name: 'P1', description: 'd1', systemPrompt: 'sp1', scope: 'coordinator', createdAt: 1, updatedAt: 1 },
      ]);
  ...
  describe('fetchSkills', () => {
    it('should fetch skills and mark them as coordinator scope', async () => {
      ...
      setupMockHttpResponse(200, [
        { id: 's1', name: 'S1', description: 'd1', trigger: 't1', body: 'b1', scope: 'coordinator', createdAt: 1, updatedAt: 1 },
      ]);
  ...
  describe('relayMessage', () => {
    it('should relay a message and return success', async () => {
      ...
      const result = await client.relayMessage('target-agent', 'source-agent', '{"text":"hello"}');
    ...
    it('should return success false on failure', async () => {
      ...
      const result = await client.relayMessage('target-agent', 'source-agent', 'body');
  ...
  describe('pushKnowledge', () => {
    it('should push knowledge to coordinator', async () => {
      ...
      await client.pushKnowledge({
        id: 'k1', type: 'fact', key: 'test', value: '{}',
        sourceBeaconId: null, sourceAgentId: null, confidence: 1.0, createdAt: 1, updatedAt: 1,
      });
  ...
  describe('pullKnowledge', () => {
    it('should pull knowledge from coordinator', async () => {
      ...
      setupMockHttpResponse(200, [
        { id: 'k1', type: 'fact', key: 'test', value: '{}', sourceBeaconId: null, sourceAgentId: null, confidence: 1.0, createdAt: 1, updatedAt: 1 },
      ]);
    ...
    it('should return empty array on failure', async () => {
      ...
      mockRequest.mockImplementation((_opts: any, _callback: (res: any) => void) => {
        const req = new EventEmitter() as any;
        req.write = vi.fn();
        req.end = vi.fn();
  ...
  describe('pushEvents', () => {
    it('should push events to coordinator', async () => {
      ...
      await client.pushEvents([{
        id: 'e1', sessionId: 's1', type: 'msg', createdAt: Date.now(),
      }]);
      expect(mockRequest).toHaveBeenCalled();
    });
  });
