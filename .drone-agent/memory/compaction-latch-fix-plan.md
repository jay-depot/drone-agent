...
updated: 2026-07-08T03:43:50.951Z
---

# Plan: Fix the permanent `compactionInFlight` latch bug

## Summary
The compaction plugin's re-entrancy guard (`compactionInFlight`) is latched `true` and never released on early-exit paths of `maybeCompact`. Every shell fires `onBeforePrompt` *before* the user message is appended (`interactive.ts`, `index.tsx`, `app.tsx`), so the first prompt of a session hits the `turns.length === 0` early return with the flag already set — and it stays set forever. Compaction never runs; the user only sees the conversation service's crude `ensureSafeBudget` drop (`conversation-service.ts:203`). Fix: wrap the flag-setting logic in `try/finally` (via a shared helper) for **both** the registered hook and the `forceEvaluate` capability, and replace a bogus regression test with real same-instance coverage.
...
All 7 steps executed and the full validation gate is green.

**Source changes (`drone-agent/src/plugins/compaction/index.ts`):**
- Extracted `runCompaction(context, budgetService, systemPrompt)` module-level helper.
- Wrapped `hookBody` in `try { await runCompaction(...) } finally { context.compactionInFlight.value = false; }`.
- Wrapped `forceEvaluate` identically. The two top guards (`!config.enabled`, already in-flight) remain plain early returns before the flag is set, so their behavior is unchanged.

**Test changes (`drone-agent/test/compaction.test.ts`):**
- Rewrote `resets compactionInFlight after the empty-turns early return` to reuse ONE `capture`/`sessionManager` (deleted the smallProvider/smallPlugin/smallCapture blocks) — now genuinely exercises the latch persisting on a single instance.
- Added `releases the latch after the empty-turns early return in the real runtime sequence` driving `onBeforePrompt` (empty session, latch latches) then `onAfterToolCall` after appending 6 long tool-result turns on the same instance.

**Verification (guard proof):**
- Stashed the fixed `index.ts` and re-ran the test file against the ORIGINAL code: both new tests FAILED (`expected "spy" to be called 1 times, but got 0 times`) — confirming they guard the bug.
- Restored the fix: all 14 compaction tests pass.
- Full gate: `pnpm typecheck` (all packages Done), `pnpm lint` (clean), `pnpm test` (1250 passed / 83 files), LSP clean for both files (only a pre-existing `hint` at line 24 of index.ts, untouched by this change).

**Commit:** `d417554` — "fix(compaction): release permanent compactionInFlight latch on early exits"