...
updated: 2026-08-31T19:59:30.777Z
---
...
# ✅ COMPLETION SUMMARY (executed 2026-08-31, branch feat/swarm-memory-rag)

**Status: ALL STEPS S1–S13 COMPLETE. Full validation green: `pnpm -r run build` 0 errors, `pnpm -r run typecheck` 0 errors (8 packages), `pnpm lint` 0 problems, `pnpm test` 2622 passed / 189 files (14 skipped by design). Working tree clean; 9 commits on the branch (86b208b…dbbb686).**

## Commits
- `86b208b` pre-work: resolved all 16 pre-existing workspace LSP diagnostics (spawn proxy honest null-types, mock typing, MakeReply intersection, gateway literal types)
- `b7c1bb6`+`9ab0425` Phase A (S1–S4): wiki `wiki_sources`/`wiki_chunks`/`wiki_vec_chunks` tables + `db/wiki-chunks.ts` (10 tests); `wiki-indexer.ts` with per-origin set-difference reconcile; `wiki-index-support.ts` (collector + guarded cycle + reindex hooks + 5-min sweep in beacon main); `GET /wiki/semantic-search` + `POST /wiki/reindex`; fixed stale `drone-swarm-common/wiki-storage` subpath imports (unresolvable under vitest — why wiki routes had no tests before)
- `41bcf37` S5: `swarm.memory` config section + deepMerge + tests
- `fa69edb` S6: origin-tagged all-versions reads + `?scope=beacon|coordinator` + origin-tagged list/search; restored `/wiki/search` keyword route the S4 rewrite dropped
- `3711025` S7–S8: `ConversationWindowTracker` (event-driven — see deviation below) + `filterForQuery` + `buildQueryInputs` (client-side token budget, chunkText segmentation, current-query-always-first, sha256 debounce)
- `9973e9f` S9–S11: `SwarmMemoryRetriever` (debounce, max-score merge, anchor boosts, failure-keeps-cache, zero-network-when-disabled), `memory-fragment.ts` (`# Swarm Memory (wiki)` advertise+recall, cache-only render), swarm plugin wiring (tracker on onConversationEvent, fire-and-forget refresh in onBeforePrompt), `--debug swarm-memory` docs row, `/swarm-memory status|refresh|session-scope`
- `6d018da` S12: ADR 179 + project wiki index + AGENTS.md memory-tool correction
- `dbbb686` S13 lint cleanup (unused imports/vars) after prettier reformat

## Deviations from the plan (all improvements, documented in ADR 179)
1. **S7 window source**: the plan's `assembleWindow(sessionManager.getMessages(), …)` cannot work — `DronePluginRegistration` exposes no session-manager reads. Replaced with `ConversationWindowTracker` fed by `onConversationEvent` events; `roundComplete` (a real engine event, ADR 169 precedent) closes rounds. Semantics are stricter and test-verified: assistant messages overwrite the in-flight round's final response; mid-round user messages are steering.
2. **S2/S3 reconcile contract hardened**: `indexWiki(pages, reconcileOrigins?)` defaults to reconciling BOTH origins; the coordinator-failure guard narrows reconcile to `['beacon']` explicitly in `runWikiIndexCycle`. An initial S3 change defaulted reconcile scope to "origins present in pages", which made empty authoritative sets no-ops — caught by the S2 deletion tests during a combined test run and fixed (default = both origins; narrowing is the caller's explicit duty).
3. **S4 rewrite drop**: the first `routes/wiki.ts` rewrite accidentally dropped the `/wiki/search` keyword route; restored in S6 with origin tags.
4. **vitest subpath specifier limitation**: `drone-swarm-common/wiki-storage` dynamic imports don't resolve under the shared vitest config (root-only resolution) — wiki routes now import via the root package specifier; this is why wiki routes previously had no tests.
5. `drone-agent` gained `drone-swarm-common` as a workspace dependency for `chunkText`.
6. wiki-librarian persona guidance was NOT edited (librarian persona lives on coordinator side; ADR 179 + decision-index row serve as the durable guidance; flag for follow-up session).
7. S13 step 5 (manual smoke with a live beacon) not executed in this session — no live beacon/coordinator environment was running; covered instead by 24 route/indexer tests incl. the deletion-tightness trio. Recommend a live smoke before enabling `swarm.memory.enabled=true` anywhere.

## Validation criteria scorecard
- Unit tests S1–S12: ✅ (all green in `pnpm test`)
- Deletion-tightness trio: ✅ (wiki-chunks + wiki-indexer + wiki-index-support tests)
- enabled:false ⇒ zero fetch calls + fragment false: ✅ (memory-retrieval/memory-fragment tests)
- Render is I/O-free: ✅ (render reads cache only; verified by test structure)
- Segment builder invariants (current query first+verbatim; hash determinism): ✅
- LSP/typecheck: ✅ zero errors project-wide (workspace was pre-cleaned in 86b208b)
- Lint: ✅ (after cleanup pass; prettier reformat followed by typecheck re-verification)
- `POST /wiki/reindex` + `GET /wiki/semantic-search` route tests: ✅
- Config deepMerge (partial `enabled:true` + anchors override): ✅ (drone-core index.test.ts)