...
**Where the redundancy lives:** `swarm__wiki_write` stores the librarian's `content` as the page body. `readPage()` in `drone-swarm-common/src/wiki-storage.ts` strips only the _outer_ frontmatter that `writePage()` builds, so `page.content` still begins with the librarian's own `---\n...\n---\n` block. `WikiMarkdown` (`src/components/wiki-markdown.tsx`) is the single renderer (used only on the read view) and currently renders the whole string, so the redundant block shows as raw text.
...
All steps executed on branch `feat/memory-wiki-browser-improvements`. Commit `5ab5c0e`.

### What was done
- Added `splitFrontmatter` helper (`drone-coordinator-ui/src/lib/wiki-frontmatter.ts`) using the same leading-frontmatter regex shape as `wiki-storage.ts`. Returns `{ frontmatter, body }`; `body` is `trimStart()`-ed to match the storage layer's convention.
- Added 4 helper tests (`wiki-frontmatter.test.ts`): splits a leading block, returns content unchanged without one, only splits a leading block (not a later `---`), and handles an empty body.
- `WikiMarkdown` now calls `splitFrontmatter(children)` and, when a frontmatter block is present, renders a native `<details>` (closed by default) with a "Metadata (YAML frontmatter)" summary and the raw YAML in a `<pre>`. The body renders through the existing markdown pipeline unchanged.
- Extended `WikiMarkdown` tests (3 new): collapsed by default (YAML hidden, body visible), reveals raw YAML on summary click, and renders normally without a frontmatter block.
- Updated the two swarm-memory fragment tests (`memory-fragment.test.ts`, `memory-trigger.test.ts`) to match the reworded fragment text committed in the baseline (`a88107c`).

### Validation
- LSP clean in `drone-coordinator-ui` (zero diagnostics on all touched files).
- `pnpm -r run typecheck` passes.
- `pnpm lint` (eslint + prettier) passes; prettier-only collateral on unrelated files reverted to keep the commit focused.
- `pnpm -r run build` passes.
- `pnpm test` passes: 2701 passed / 14 skipped (193 files). UI suite: 49 passed (11 files), including the 4 new helper tests and 3 new markdown tests.

### Notes
- The `pnpm -r run test` recursive form fails in `drone-core` because it has a `test` script but no test files — a pre-existing infra quirk; the canonical suite is the root `pnpm test`.
- The `<details>`/`<summary>` approach is state-free (no React state), so the collapse test is straightforward: assert YAML absent by default, present after clicking the summary.
