...
updated: 2026-08-17T15:04:40.172Z
---
...
- [ ] `rename` tool, when given ambiguous text/symbol, returns reference IDs with snippets and `suggestedSurroundingText`
- [ ] `code_action` tool, when given ambiguous text/symbol, returns reference IDs with snippets and `suggestedSurroundingText`
- [ ] `rename` tool, when given a `referenceId` from a previous ambiguous resolution, resolves and executes
- [ ] `completion` tool returns a `snippet` field with code context around the query position
- [ ] `buildAutoExpansion` returns snippets for all locations, not just one per file
- [ ] `resolveSymbolPosition` applies `surroundingText` to workspace symbol matches
- [ ] `readFileSnippet` has no `column` parameter
- [ ] `diagnostics` tool schema has no `surroundingText` field
- [ ] `go_to`, `inspect`, `completion`, `call_hierarchy` all call `refreshIfNeeded()` before position resolution
- [ ] `AmbiguousPositionError` is exported from `drone-core`
