...
| Command            | Behaviour                                                                                                                                                                                                                                                                                                                           |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/steer <message>` | Opt-in **mid-round injection**. Inserts a user-role turn into the _currently in-flight_ round at the next tool-loop boundary. This re-introduces the ADR-040 "mid-round absorption" that ADR 215 deliberately removed, but only when the user **explicitly** asks for it. When no turn is in flight, it degrades to a normal round. |
...
ADR 215 / PR #106 unified deferred user intent into one ordered `pendingEntries` queue drained at exactly two round boundaries, and **deleted mid-round absorption**. That made queued text correct and predictable, but removed any way to influence a round _while it runs_. `CONTEXT.md` still defines a Round as containing "zero or more user steering message turns", and the `ConversationWindowTracker` still classifies such turns — the vocabulary survived, the mechanism did not.

- `/steer` restores steering as an explicit, opt-in control (a _consequence of the new queuing logic_: because plain text now always becomes its own later round, steering needs its own command).
...
1. **LSP diagnostics are clean** on every touched file (`lsp__get_diagnostics`), with zero new errors. _Exception:_ the 8 pre-existing stale-`dist` errors in `drone-beacon/test/wiki-indexer.test.ts` (`wordCount`/`linkCount` missing on `DroneWikiPageMeta`) are out of scope — re-confirm after `pnpm -r run build` that they are unrelated to this change and unchanged in count.
...
6. **`/steer` while busy:** a `/steer <msg>` typed mid-round produces a `[steering: <msg>]` notice, and the _next_ LLM request's messages contain `<msg>` as a user turn **before** the round's final assistant reply is produced.
...
Executed on branch `feat/btw-and-steer-commands`. Implementation commit `529ea20b`.

### What was built (all 15 locked decisions honoured)

- **`/steer <message>`** — `busyBehavior: true`; a dedicated `steeringMessages: string[]` buffer drained at the loop top (after the soft-cancel check, before `getLlmTools()`); each absorbed message appends a user turn and emits `userMessage`. When idle it degrades to a normal round with `onBeforePrompt`/`onAfterToolCall`. Absorbing resets the degeneracy guards but not `iterationCount`. Un-absorbed steers are discarded at round end with `[steering: discarded late steering message: "..."]`; a successful injection logs `[steering: <msg>]` (emitted at absorption time, not enqueue time).
- **`/btw <question>`** — `busyBehavior: true`; re-assembles header + session + footer context plus a framing line, sends with **no tools**, emits one `{ kind: 'aside', question, answer }` event, discards the copy. Session history untouched. Uses the active model + the shared `resolveEffectiveReasoningLevel` (session override → model entry → llm config). Runs `prepareRequestMessages` only, skips `describeUndescribedImages`.
- **Surfaces**: TUI committed `aside` scrollback entry (new `ChatEntry` kind); plain + swarm-listen output handlers; coordinator `KEPT_EVENT_KINDS` + `parseEvent` + `renderEvent` (`[aside Q]`/`[aside A]`).
- **Wiring**: both methods on all three slash-context builders (TUI, bare, readline); `clearSession` flushes `steeringMessages`.

### Review pass (persona `review`) — 2 blockers + 4 should-fixes fixed before commit

1. **Test LSP red** — `slash-steer-btw.test.ts` passed partial conversation literals; fixed with a full default + `Partial` overrides. (Vitest/esbuild does not typecheck tests, so `pnpm test` alone would have missed this; `tsc -p tsconfig.test.json` is the gate.)
2. **Unhandled rejection** — a `/btw` or idle-`/steer` LLM failure could crash the process (slash handlers `await` the promise; nothing upstream catches). Both now catch and emit an `error` event.
3. **Notice truthfulness** — the success notice fired at enqueue time, contradicting the discard notice for the same message; moved to absorption time.
4. **Reasoning level** — `askAside` used the raw session variable, bypassing the configured fallback; extracted `resolveEffectiveReasoningLevel` and reused it in the main loop.
5. **Coverage** — added tests for guard-reset (steer escapes an identical-call loop) and depth-limit-unaffected (steer does not extend `iterationCount`).
6. Nits 6/11 also fixed (TUI type members; idle-`steerMessage` `try/finally`).

Accepted-as-is nits: idle-`/btw` shares the last round's `correlationId` in the transcript; aside events flush with the next tool round; `runJsonListenMode`'s per-call handler omits `aside` (unreachable there — adding it would be dead code); `conversation-service.ts` grew to ~1687 lines (pre-existing, split still deferred).

### Validation (all green)

- LSP clean (the 8 pre-existing `wiki-indexer.test.ts` errors were stale-`dist` and are gone after `pnpm -r run build`).
- `pnpm -r run build` exit 0; `pnpm lint` exit 0 (root is canonical — `pnpm -r run lint` fails, drone-core has no lint script); `pnpm test` 3079 passed / 14 skipped / 0 failed; `npx tsc -p tsconfig.test.json` exit 0.
- Manual TUI smoke (feature gate 14) left to the user.

### Notable gotchas hit during execution

- **`file__apply_diff` silently no-op'd several hunks** (reported `patched: true` but did not apply) — happened 4×; a tail-fragment corruption in `transcript.ts` was a 5th. Every apply must be verified with `file__read` + a typecheck/test run.
- **`pnpm lint` reformats unrelated files** (it runs `prettier --write .`). The run rewrote `pnpm-lock.yaml` (5863-line churn, prettier 3.9.6 style drift), two unrelated test files, and unrelated memory/insight files. These were reverted with `git checkout --` before the feature commit; use `prettier --check` (not `pnpm lint`) to verify formatting without side effects.
