 ...
 */
async function generateTlsCertificateWithOpenssl(
  configDir: string,
  ...
  try {
    ...
    // execFileAsync with an explicit argv array avoids shell injection from commonName/tempDir.
    await execFileAsync(
      'openssl',
      [
...
export async function loadOrCreateTlsIdentity(
  ...
  if (fs.existsSync(certPath) && fs.existsSync(keyPath)) {
    try {
      const certPem = await fs.promises.readFile(certPath, 'utf-8');
      const keyPem = await fs.promises.readFile(keyPath, 'utf-8');
  ...
  await fs.promises.writeFile(keyPath, keyPem);
