...
import type { Persona, Skill, Beacon, CreatePersonaRequest, CreateSkillRequest, RegisterBeaconRequest, BeaconSession, CreateSessionRequest } from "./types.js";
...
export function createBeaconSession(beaconId: string, req: CreateSessionRequest): BeaconSession {
  const now = Date.now();
  ...
  logger.info(`Created beacon session: ${session.id} for agent ${session.agentId}`);
  return session;
}

export function getBeaconSession(beaconId: string, agentId: string): BeaconSession | undefined {
  const stmt = getDatabase().prepare("SELECT * FROM beacon_sessions WHERE beacon_id = ? AND agent_id = ? AND disconnected_at IS NULL");
...
export function listBeaconSessions(beaconId: string): BeaconSession[] {
  const stmt = getDatabase().prepare("SELECT * FROM beacon_sessions WHERE beacon_id = ? ORDER BY connected_at DESC");
...
export function endBeaconSession(beaconId: string, agentId: string, disconnectedAt: number, durationMs: number): BeaconSession | undefined {
  ...
  logger.info(`Ended beacon session for agent ${agentId}, duration: ${durationMs}ms`);
  
  // Fetch and return updated session
  const stmt2 = getDatabase().prepare("SELECT * FROM beacon_sessions WHERE beacon_id = ? AND agent_id = ?");
...
export function deleteBeaconSession(beaconId: string, agentId: string): boolean {
  const stmt = getDatabase().prepare("DELETE FROM beacon_sessions WHERE beacon_id = ? AND agent_id = ?");
