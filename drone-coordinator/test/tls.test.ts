...
describe('Coordinator TLS', () => {
  ...
  itIfOpenssl('should generate a new TLS identity when files do not exist', async () => {
    ...
    const certExists = await readFile(identity.certPath, 'utf-8').then(() => true).catch(() => false);
    const keyExists = await readFile(identity.keyPath, 'utf-8').then(() => true).catch(() => false);
