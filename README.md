...
- `drone-coordinator`: The `drone-coordinator` will be the layer that manages communication between `drone-beacon` instances. It will also provide a web UI to monitor any running agents in the swarm, and task-manage them as well as interact with a coordinator management persona. The coordinator also provides swarm-wide personas, skills, and memory store.
...
The `skill-library/` directory contains reusable skill `.md` files that can be copied into `~/.drone-agent/skills/` to extend the agent's capabilities. These are not part of the core package but are provided as a starting point for common tasks like porting agents and skills from other frameworks.
