		...
		case 'paragraph':
			return (
				<Text color={textColor}>
					{token.tokens?.map((t: any, i: number) => renderInlineToken(t, color)).flat()}
				</Text>
			);
		...
		case 'strong':
			return [
				<Text bold key={token.raw}>
					{token.tokens?.flatMap((t: any) => renderInlineToken(t, defaultColor))}
				</Text>,
			];
...
function renderBlockquote(
	...
	const content = token.text || token.tokens?.flatMap((t: any) => renderInlineToken(t, color)).join('') || '';

	return (
		<Box flexDirection="row">
			<Text color="gray">│ </Text>
			<Box flexDirection="column">
				<Text italic color={color}>{content}</Text>
			</Box>
		</Box>
	);
}
